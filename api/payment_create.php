...
try{$pay=new WechatPay();$pay->assertReady();DB::createPaymentOrder($number,$amount,$description);$url=$pay->createNative($number,$amount,$description);echo json_encode(['ok'=>true,'order_no'=>$number,'code_url'=>$url],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);}catch(Throwable $e){error_log('Payment create failed: '.$e->getMessage());http_response_code(500);echo json_encode(['ok'=>false,'message'=>$e->getMessage()],JSON_UNESCAPED_UNICODE);}
