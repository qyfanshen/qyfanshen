...
### 核心功能
- AI 智能对话：集成 OpenAI 兼容接口（可在 .env 切换为 DeepSeek / 通义等）
- 微信支付全流程：下单 / 异步回调 / 状态查询
- 用户认证：bcrypt 加密 + CSRF 防护 + 接口限流
- 管理后台：登录、消息、支付订单、商品管理
- 账户中心：资料、订单、历史、安全登出
- 隐私与安全：HTTPS 强制、安全头、敏感文件隔离

### 技术特性
- 现代化技术栈：PHP 8+ · MySQL 5.7+ / SQLite · jQuery · Nginx/Apache
